...
TorrentRendererFull.prototype =
{
	...
	getProgressDetails: function(controller, t)
	{
		...
		if (is_done) {
			...
			// patch for auto-delete begin
			var isKeep = (t.getName().substr(0,6)=='[keep]');
			var fields = ['id'].concat(Torrent.Fields.StatsExtra);
			controller.updateTorrents(t.getId(), fields);
			var now = Date.now();
			var d = now/1000 - t.getLastActivity();
			var isOld = (d > (24 * 3600));  // 24 hours is too old
			var str;
			if(d < 0)
				str = none;
			else if(d < 5)
				...
				str = Transmission.fmt.timeInterval(d) + ' ago';
			c.push(', Last Activity (',
			       str,
			       ')');
			if(isKeep) {
				c.push(' Will keep. ');
				...
				if(isOld) {
					var ts=[t];
					controller.removeTorrentsAndData(ts);
				}
				c.push(' Add "[keep]" at beginning of torrent name to keep it from auto-delete. ');
			}
			// patch for auto-delete end
